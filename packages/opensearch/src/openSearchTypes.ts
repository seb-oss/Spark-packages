...
export type StandardAnalyzer = 'standard' | 'simple' | 'whitespace' | 'stop' | 'keyword' | 'pattern' | 'fingerprint'
export type LanguageAnalyzer = 'arabic' | 'armenian' | 'basque' | 'bengali' | 'brazilian' | 'bulgarian' | 'catalan' | 'czech' | 'danish' | 'dutch' | 'english' | 'estonian' | 'finnish' | 'french' | 'galician' | 'german' | 'greek' | 'hindi' | 'hungarian' | 'indonesian' | 'irish' | 'italian' | 'latvian' | 'lithuanian' | 'norwegian' | 'persian' | 'portuguese' | 'romanian' | 'russian' | 'sorani' | 'spanish' | 'swedish' | 'turkish' | 'thai'
...
export type Aggregations<T> = { [key: string]: { field?: keyof NestedFields<T> } }
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
export type ScriptScore = { script: { source: string, params?: { [key: string]: any } } }
// biome-ignore lint/complexity/noBannedTypes: <explanation>
export type Highlight<T> = { fields: { [P in keyof NestedFields<T>]?: {} } }
export type Prefix<T> = { [P in keyof NestedFields<T>]?: NestedFields<T>[P] | { value: NestedFields<T>[P]; boost?: number } }
export type Wildcard<T> = { [P in keyof NestedFields<T>]?: NestedFields<T>[P] | { value: NestedFields<T>[P]; boost?: number } }
export type MatchPhrase<T> = { [P in keyof NestedFields<T>]?: NestedFields<T>[P] | { query: NestedFields<T>[P]; analyzer?: string; slop?: number; boost?: number } }
export type MatchPhrasePrefix<T> = { [P in keyof NestedFields<T>]?: NestedFields<T>[P] | { query: NestedFields<T>[P]; max_expansions?: number; slop?: number; boost?: number } }
...
export type OpenSearchQueryBody<T extends { id: string }, K = T> = K extends DeepPartial<T> ? {
  query: {
...
export type Sort<T> = { [P in keyof NestedFields<T>]?: { order: 'asc' | 'desc' } }[]

export type OpenSearchQuery<T extends { id: string }, K = T> = RequestParams.Search<OpenSearchQueryBody<T, K>> & K extends DeepPartial<T> ? {
  index: string
...
export type OpenSearchQueryResult<K> = { results: K[]; total: number; from: number; size: number }

// Define a type for basic Elasticsearch field types
export type BasicOpenSearchFieldTypes = 'text' | 'keyword' | 'long' | 'integer' | 'short' | 'byte' | 'double' | 'float' | 'date' | 'boolean' | 'binary'
...
export type NestedFieldOptions<T> = {
  // biome-ignore lint/suspicious/noExplicitAny: <explanation>
  [P in keyof T]?: T[P] extends object ? NestedFieldOptions<T[P]> : FieldOptions<any>
}
