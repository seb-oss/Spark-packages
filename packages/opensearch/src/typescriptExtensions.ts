...
export type NestedPaths<T> = T extends object
  ? {
      [K in keyof T]: T[K] extends string | number | boolean | Date
        ? K
        : T[K] extends object
          ? `${K & string}.${NestedPaths<T[K]> & string}`
          : never
    }[keyof T]
  : never
...
export type NestedStringPaths<T, Prefix extends string = ''> = T extends object
  ? {
      [K in keyof T & string]: NestedStringPaths<
        T[K],
...
export type NestedNumberPaths<T> = T extends object
  ? {
      [K in keyof T]: T[K] extends number
        ? K
        : T[K] extends object
          ? `${K & string}.${NestedNumberPaths<T[K]> & string}`
          : never
    }[keyof T]
  : never
...
export type FieldPath<T, Prefix extends string = ''> = T extends
  | Primitive
  | Array<Primitive>
  ? never
  : {
      [K in keyof T]-?:
        | `${Prefix}${K & string}`
        | (T[K] extends Primitive | Array<Primitive>
            ? never
            : `${Prefix}${K & string}.*`)
    }[keyof T]

export type SubstringOf<T extends string> =
