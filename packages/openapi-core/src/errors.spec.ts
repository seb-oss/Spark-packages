...
describe('fromAxiosError: ', async () => {
  test('returns default HttpError (500) when response status is not available', () => {
    ...
    expect((returnedError.cause as Error).name).toBe('AxiosError')
    expect((returnedError.cause as Error).stack).toBeDefined()
    expect((returnedError.cause as Error).cause).toStrictEqual({})
  })
  ...
  test('returns appropriate HttpError (404) based on response status and data', () => {
    ...
    expect((returnedError.cause as Error).cause).toStrictEqual(response.data)
  })
